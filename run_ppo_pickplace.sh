...
echo "环境数: 64"
echo "批次大小: 640"
echo "最大步数: 100 steps/episode"
echo "环境: PickSingleYCB-v1 (支持多种形状物体：YCB数据集)"
...
python -u ${SRC_FILE} \
    ...
    --config-name maniskill_ppo_pickplace \
    2>&1 | python3 /mnt/RL/yzy/clean_output.py | tee /mnt/RL/yzy/recordings/training_log_pickplace.txt

TRAIN_EXIT_CODE=$?

echo "========================================"
echo "训练完成!"
echo "日志保存在: /mnt/RL/yzy/recordings/training_log_pickplace.txt"
